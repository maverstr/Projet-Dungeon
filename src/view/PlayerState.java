...
public class PlayerState extends JPanel { // Jpanel for Player Stats and
	...
	public PlayerState() {
		...
		healthBar = new JProgressBar(0,
				5); /* TODO: Should get player *maxHealth* iso 5 */
		healthBar.setString("Health");
		healthBar.setStringPainted(true);
		healthBar.setBounds(10, 10, 180,
				20); /*
	...
	public void redraw(Player p) {
		try {
			this.healthBar.setValue(p.getHealth()); // update healthBar at each
													// redraw (Window.update)
			this.inventory = p.getInventory(); // update the inventory list to
												// draw
			this.repaint();
		} catch (NullPointerException e) { // At the beginning player may not be
											// created -> nullpointer
